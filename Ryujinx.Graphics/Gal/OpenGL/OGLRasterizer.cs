...
namespace Ryujinx.Graphics.Gal.OpenGL
{
    public class OGLRasterizer : IGalRasterizer
    {
        ...
        public void CreateVbo(long Key, long Size, IntPtr Address)
        {
            int Handle = GL.GenBuffer();

            VboCache.AddOrUpdate(Key, Handle, (uint)Size);

            IntPtr Length = new IntPtr(Size);

            GL.BindBuffer(BufferTarget.ArrayBuffer, Handle);
            GL.BufferData(BufferTarget.ArrayBuffer, Length, Address, BufferUsageHint.StreamDraw);
        }

        public void CreateIbo(long Key, long Size, IntPtr Address)
        {
            int Handle = GL.GenBuffer();

            IboCache.AddOrUpdate(Key, Handle, (uint)Size);

            IntPtr Length = new IntPtr(Size);

            GL.BindBuffer(BufferTarget.ElementArrayBuffer, Handle);
            GL.BufferData(BufferTarget.ElementArrayBuffer, Length, Address, BufferUsageHint.StreamDraw);
        }
